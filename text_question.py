...
phonenumber = "your_phonenumber" #Don't forget area code
...
def fetch():  #Fetch and/or refresh any new incoming messages
    ...
    con = imaplib.IMAP4_SSL(imap) #Establish imap connection
    con.login(user,pas) #Log into email server
    con.select('INBOX') #Grab default inbox
    result, data = con.fetch(b'3','(RFC822)') #Grab message in byte form
    raw = email.message_from_bytes(data[0][1]) #Turn message into text
